...
let 
	...
	g = [0 0 1 1 0 0]
	b = [1 1 1 1 0 0]
	R,G,B = j*r, j*g, j*b #다 matrix가 됨~~
	#colorview(RGB,stack([R,G,B],dims=1))
	colorview(RGB,R,G,B) #위 코드랑 이번 줄이랑 같음~
end 
...
# ╔═╡ b0a5539c-2c6a-414c-82e5-50228e100eb0
md"""
위의 숫자들을 어떻게 찾나요 ... -> SVD
"""
...
U,d,V = svd(X) #언패킹해서 받습니다.

# ╔═╡ 8622ceb0-896f-4d69-8184-d7bcb9674645
d #고윳값이 리스트로 튀어나옴.

# ╔═╡ b06dd7fa-ed1c-4f24-a925-5ca4a80685a8
Diagonal(d) #우리는 근데 행렬이 필요함.. Diagonal로 대각행렬 만듦
...
let
	...
	#---#
	U,d,V = svd(X)
	U1,U2,U3 = eachcol(U) #야가다를 방지해줌... 열의 수 만큼 열벡터를 반환해줌.
	d1,d2,d3 = d
	V1,V2,V3 = eachcol(V)
	#---#
	[X U1*d1*V1'+U2*d2*V2'+U3*d3*V3'] #셋다 더해야 비슷함.. 같음..위에서 다룬 내용
end 
...
let 
	...
	colorview(RGB, stack([R,G,B],dims=1)) #그냥 그리스 국기 나옴.
	scatter(1:9,r,color="red")
	scatter!(1:9,g,color="green")
	scatter!(1:9,b,color="blue") #고윳값들을 scatter plot한건데 3개만 값이 있고 4번쨰부터는 모두 0이다.. 그러면 블락 매트릭스를 만들면 사실상 3 대각 원소만 값이 있고 나머지는 다 0이다.
	#colorview(RGB, R,G,B)
end
...
manifest_format = "2.0"
project_hash = "d0db1ab7ab4fdef045acf39429fe7c74c7585c72"
