...
# ╔═╡ 64bcbb92-f7b0-4d2e-a9dc-6e283e631603
k(x,y) = x+y #함수 1 x와 y를 입력 받는 함수 오브젝트 k 

# ╔═╡ 7a13d72a-c552-44b4-a795-f01ba5f99ed5
k(2,-2) #함수1 출력
...
g = (x -> 2x) #일변량 함수 람다를 다루기 위해 썼던 초안이 그대로 적용된다! + object로 지정
...
# ╔═╡ 3eeb67e6-b474-4a93-849e-30319fe1607c
md"
* 람다 함수의 지정이 정말 간편 
"
...
function h(x)
	return ℯ^x / (1+ ℯ^x) #sigmoid Function
end
...
# ╔═╡ 1e25d3e8-84da-464e-a972-9fc7677e304e
md"
`스포일러:` 사실은 로지스틱 model이다..!"

# ╔═╡ 1a79bbf1-7927-4295-a6ff-2a187ad0e6cf
a = @bind a Slider(-5:0.1:5,show_value=true)
#object이름을 일치시키면 호출해도 숫자만 나옴
#원래 충돌이 생길수도 있다는데 새 버전이라 괜찮은듯 하다..

# ╔═╡ eb821001-a2fb-4cb7-8ad2-fc65f836d8f3
a #바인드된 값만 출력합니다!.

# ╔═╡ 8b883f16-e4a1-4cc2-9d6a-5239275a6d1b
sldr = @bind c Slider(-1:0.1:1,show_value = true) #이름을 다르게 저장하면

# ╔═╡ 1ef7b7d8-b632-48b5-beb9-f780679d2214
sldr #결과가 그냥 슬라이더 오브젝트
...
let 
	l(x) = a*x+b #liner function을 만들고
	plot(h ∘ l, -20,20, legend=false) #activation function에 대입한 것..
	xlims!(-25,25) #기존에 구현된 plot에 이어서 작동시키겠다!
	title!("l(x)=($a)x+($b)")
end

# ╔═╡ 8ef4c199-aba2-48c7-847b-b5cc5183bcc0
md"a 는 시그모이드 함수가 얼마나 급하게 변하는지\
b는 시그모이드 함수의 shifting"

# ╔═╡ 7fdd85fe-4e1f-4717-8f0f-a6a1aef28b42
md"합성함수 연산자는 \circ ⋆⋆⋆"
...
# ╔═╡ 00cc3690-a5f0-417b-9073-ffec18d3c7f8
[1,2,3] |> mean #리스트 전체에다가 평균이라는 함수 적용!

# ╔═╡ ab1f008c-5590-47c8-9da5-57defc49fc0d
[1,2,3] .|> mean #앞에 점을 찍었더니 각 원소별로 평균이 적용 되었다.

# ╔═╡ d8a5e876-8ab3-47c7-85b9-3966f935e042
[1,2,3] + [3,4,-2]

# ╔═╡ eeff8fd9-0121-486b-a593-1009c3be160f
[1,2,3] + 1 #이런건 그냥 에러가 나와버림
#우리의 생각... R에선 리스트 원소별로 덧셈을 알아서 해주던데...

# ╔═╡ c8416fc8-2a59-45cd-a929-ed4dac57d43c
[1,2,3] .+ 1 #각 원소에 대해 1을 더하는 연산을 수행하겠다.
#브로드 캐스팅~

# ╔═╡ 9baff50c-545d-46c3-99f3-09aeb0821b9a
2 *[1,2,3] #상수배는 브로드캐스팅 됨

# ╔═╡ 11ad4b57-2c48-46e3-85b1-3c77555c8e74
[1,2,3] |> (x -> 2x) #이건 그냥 리스트를 2배 한거임

# ╔═╡ ed657fa5-c800-4126-9869-52f1df7dcb88
[1,2,3] .|> (x -> 2x) #이건 각 원소별로 2배씩 취한것 사실상 같다..

# ╔═╡ baa345fd-079f-4b09-97f7-39f5455ae84f
[1,2,3] |> (x->x^2) |> (x-> x-1) |> mean #오류.
#뭐가 오류일까..? 엘레멘트 와이즈하게 제곱을 취해주고 싶은데
#제곱도 원소에 적용되는 함수고 x-1도 원소에 적용되는 함수라
#브로드캐스팅을 하겠다고 선언을 해주어야 한다.

# ╔═╡ 2fba95f8-eecd-4737-bbcc-7a6a3c79ce58
[1,2,3] .|> (x->x^2) .|> (x-> x-1) |> mean #앞 두개는 점을 안찍으면 문제생김.

# ╔═╡ ac1e7e15-cd86-4470-8445-36e356cfd08f
@. [1,2,3] + 1 #브로드캐스팅을 선언하는 @.

# ╔═╡ 134ecfc1-abf4-45a0-a17b-9b444b464529
(@. [1,2,3] |> (x -> x^2)) |> mean #괄호로 갇힌 곳만 브로드 캐스팅,, 아닌 부분은 전체에
...
md"""
파라메터
-  $\rho$ = $(@bind ρ Slider(-0.99:0.01:0.99, show_value=true,default = 0.3))
-  $\mu_X$ = $(@bind μ₁ Slider(-1:0.1:1, show_value=true, default = 0.5))
-  $\mu_Y$ = $(@bind μ₂ Slider(-1:0.1:1, show_value=true, default = 0.5))
-  $\sigma_X$ = $(@bind σ₁ Slider(0.01:0.01:2, show_value=true, default = 1))
-  $\sigma_Y$ = $(@bind σ₂ Slider(0.01:0.01:2, show_value=true, default = 1))
"""
#이제는 해석할 수 있다... 바인드 된 애들은 숫자가 잘 들어가 있다..!

# ╔═╡ f4c66d21-e0c5-47c2-9099-d8484fd3a97b
function pdf(x,y) #pdf라는 이변량 함수를 선언하겠다.
	x = (x-μ₁)/σ₁
	y = (y-μ₂)/σ₂
	c1 = 2π*σ₁*σ₂*√(1-ρ^2)
	c2 = 2(1-ρ^2)
...
plot(p1,p2) #상당히 직관적인 코드, 그림 객체를 나란히 입력하면 된다.

# ╔═╡ 211bc5ed-6ac0-47d7-be12-26b9b731ae27
md"""
 $\theta$ = $(1+1)
""" #다소 난해한 문법.... md 안에 $를 넣어주면 코드가 구현된다?
