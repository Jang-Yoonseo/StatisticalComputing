...
# ╔═╡ 56fef0d4-7d85-4d06-8021-58ac09100ec2
let
	mu = @bind mu Slider(-3:0.1:3, show_value=true, default=0)
	sgm = @bind sgm Slider(0:0.1:3, show_value=true, default=1)
	mu,sgm
end

# ╔═╡ 00a9ffc1-78bb-47a1-b4a7-24388bd0fa74
let
	rand(Normal(mu,sgm),10000) |> (x -> histogram(x,xlim =(-20,20)))
end
...
let 
	...
	Θ = rand(N)*2π
	R = @. √(-2log(1-U))
	(@. R*cos(Θ)) |> (x -> histogram(x, label = "박스 뮬러 변환"))
	randn(N) |> (x -> histogram!(x, label = "정규분포 난수"))
	#histogram((@.R*cos(Θ)),label = "박스 뮐러 변환")
	#(@. R*cos(Θ)) |> (x -> histogram("박스 뮬러 변환")) #왜 이건 안될까 했는데 좀 답이 된듯..
end 
...
# ╔═╡ 8f65d0c7-28fb-4699-a428-5dfbd8ebb09c
let
	n1 = [mean(rand(Normal(7,5),25)) for i in 1:1000]
	n2 = rand(Normal(7,5/√25),1000)
...
let 
	N = 10000
	n = 25
	[rand(Normal(7,5), n) |> mean for i in 1:N] |> histogram #이중리스트에 평균
	rand(Normal(7,5/√n),N) |> histogram! #이론적인 표본 평균의 분포.
end 
...
let 
	...
	σ = 10
	n = 25
	X̄ = [rand(Normal(0,σ),n) |> mean for i in 1:N] #표본 평균의 분포를 난수로 관찰
	#c = X̄ |> (x -> quantile(x,0.975))
	c = quantile(X̄,0.975)
	x̄ = 67.53
	(x̄-c,x̄+c,c) #c는 그냥 퀀타일 한번 찍어본거임
end 
...
let
	x̄ = 67.53
	sim = [mean(rand(Normal(0,10),25)) for i in 1:1000]
	q = -quantile(sim,0.065)
	x̄ - q , x̄ + q
end

# ╔═╡ 254a0a12-6ec5-421d-8092-3ea6e629397b
#이론 버전
let
	x̄ = 67.53
	th = Normal(0,10/5)
...
# ╔═╡ 0749fff1-5bf2-4204-87e3-823962b19762
let
	histogram([sum(rand(Normal(0,1),4).^2) for i in 1:1000])
	histogram!(rand(Chisq(4),1000))
end	

# ╔═╡ a4ea26e9-e769-40e7-bc34-90a6b50198b2
let
	N = 10000
	[rand(Normal(0,1),4) .^2 |> sum for i in 1:N] |> histogram #카이제곱분포의 대의적 정의
...
let 
	...
	#Exp(2) 는 표준 정규분포를 제곱해서 합친건데
	#거기에 또 Exp(2)를 더하면.. 표준 정규분포를 제곱해서 네번 더한것
	#즉 카이-제곱(4)
	rand(Chisq(4),N) |> histogram! #저번 수업을 잘 기억해야.. 전개가 매끄러움
end 

# ╔═╡ c832ccb5-c6ce-48a5-81a4-286af9bee746
let
	histogram([sum(rand(Exponential(1),2)*2) for i in 1:1000])
	rand(Chisq(4),1000) |> histogram!
end
...
# ╔═╡ 7e5f5c40-59d1-40a3-a95a-e77bf3b09d7f
[sum(-2*log.(1 .- rand(3))) for i in 1:1000]
...
let 
	...
	[sum(-2*log.(1 .- rand(3))) for i in 1:10000] |> histogram!
end
...
# ╔═╡ 0da9018d-9738-4d3d-80e6-a3bac255676b
[sum(rand(Chisq(2),3)*1/2) for i in 1: 10000]

# ╔═╡ 0d4ce233-8a11-48d0-ac24-e21dcd1c6c74
let 
	N = 10000
	rand(Chisq(6),N) |> histogram
	rand(Gamma(3,2),N) |> histogram! #평균이 2인 지수분포를 3개 더함 -> 카이제곱(6)
	[sum(rand(Chisq(2),3)) for i in 1: 10000] |> histogram!
end
...
# ╔═╡ a88914aa-738a-4601-b44e-6c2e51a3eaaf
let
	rand(Gamma(3,3),10000) |> histogram
	[sum(randn(6).^2) *3/2 for i in 1:10000] |> histogram!
end
...
let 
	N = 10000
	rand(Gamma(1/2,2),N) + rand(Gamma(1/2,2),N) |> histogram #카이제곱(2) 한쪽은 카이제곱(1)
	rand(Gamma(1,2),N) |> histogram! #카이제곱(2)
end 
...
# ╔═╡ 7f043a9d-0187-4002-9611-699acf20c162
md"스스로 해결해보아요!"
...
# ╔═╡ a11da70a-3fff-4a6b-81de-1d85ccede454
let 
	N = 100000
	σ = 10
	n = 25
