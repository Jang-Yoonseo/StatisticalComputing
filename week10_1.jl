...
begin
	j = ones(200)
	Choco = [Dict("choco"=>1, "vanilla"=>0)[key] for key in ic2.type] #Dict : 파이썬에서 딕셔너리. 
	Vanilla = [Dict("choco"=>0, "vanilla"=>1)[key] for key in ic2.type]
	x = ic2.temp
	X = [j x Choco Vanilla] #n*4 matrix.
	y = ic2.sales
end

# ╔═╡ a93520fe-a35c-4136-9cf5-dc3076a60219
let
	초코만 = (ic2.type .== "choco") #부울 나옴..
...
let
	...
	#역행렬이 존재하지 않아서 회귀분석때 배운 내용을 활용할 수 없음 ...
end 
...
let 
	U,d,V = svd(X)
	d1,d2,d3,d4 = d
	β̂ = V*Diagonal([1/d1,1/d2,1/d3,0])*U'*y #3개만 중요한 설명변수... 나머지 고윳값은 버려도 됨(PCA의 아이디어.)
	@show β̂
	scatter(x,y)
	scatter!(x,X*β̂)
	# 역행렬이 존재하지 않는 경우 (가변수 회귀분석)에서는 SVD를 활용하면 쌉가능!
end 
...
let
	X1,X2,X3,X4 = eachcol(X)
	Z = [X1 X2 X4] #선형 종속을 피하기 위해 한 설명변수 삭제(가변수일 땐 가능..)
	γ̂ = inv(Z'Z)Z'y
	@show γ̂
	scatter(x,y)
	scatter!(x,Z*γ̂)
...
let 
	X1,X2,X3,X4 = eachcol(X)
	Z = [X2 .- mean(X2) X3 .- mean(X3) X4 .- mean(X4)] #센터링 : 다중 공선성을 피하기 위해
	U,d,V = svd(Z)
	d1,d2,d3 = d
	γ̂ = V * Diagonal([1/d1,1/d2,0]) * U' * y #적합.
