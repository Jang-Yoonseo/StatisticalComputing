...
let 
	...
	quantile(R,0.95) #95% quantile 값을 찾아줌!
	#반지름을 2.42정도로 잡으면 95개의 점이 포함된다
end 
...
bang, paper #슬라이더와 라디오버튼 튜플 -> 동시에 선택 가능!
...
# ╔═╡ e93e2da2-c4ab-4ed7-a7a3-3250646472fb
let
	plot(front1) #좀 재미있는 사실을 찾아냄..
	#front에는 느낌표로 추가로 주렁주렁 단 것 까지 저장됨..
	#아래 느낌표 두개를 주석처리 해보면 흰생 맹탕 산점도만 남게 됨.
end
...
begin
	if side == "앞면" 
		...
		back2 = plot(back1) #이미 X[i],Y[i] 점은 찍혀있음
		scatter!([X[i-1],X[i+1]], [Y[i-1],Y[i+1]],color="blue",alpha=0.5)
	end 
end
...
let
	...
	if side == "앞면"
		...
		x = @. r*cos(θ) + μ1 #중심이 mu1,mu2 고정되어있음
		y = @. r*sin(θ) + μ2
		plot!(x,y,color="red",linestyle=:dash)
	else 
		plot(back2)
		scatter!([X̄],[Ȳ],color="blue",marker=:cross,markersize=7)
		x = @. r*cos(θ) + X̄ #관측하는 점에 따라 반지름을 바꿈..
		y = @. r*sin(θ) + Ȳ
		plot!(x,y,color="red",linestyle=:dash)
	end 
end
...
let 
	X̄ = [rand(X,3) |> mean for i in 1:5000] #표본에서 임의로 3개의 값을 뽑음
...
# ╔═╡ 4e644db7-b1ba-4385-8448-a7127bb44ce6
let
	N = 10000
	X̄ = [mean(rand(Normal(0,1),3)) for i in 1:N]
	Ȳ = [mean(rand(Normal(0,1),3)) for i in 1:N]
	R = (X̄.^2 + Ȳ.^2) .|> (x -> √x)

	th = rand(Exponential(2/3),N) .|> (x->√(x))
...
let 
	a = quantile(Exponential(2),0.95) #qunatile에 분포를 넣으면 theorical qunatile이 나옴..
	println("Pr(R² < $(a)) = 0.95")
	println("Pr(R < $(√a)) = 0.95")
end 
...
manifest_format = "2.0"
project_hash = "44f9b58d916db890ede50aa3bc11d24b700faaa7"
