...
begin 
	X1,X2,X3 = eachcol(df) #각 열 언패킹..
	X = [X1 X2 X3] 
end

# ╔═╡ 05c609a1-7ae4-4fb2-8a1f-f6ba12511613
let
	y = 600*X1 + 5*X2 + 300*randn(n)#rand(n)은 급여의 운적인 요소..
...
let 
	N = 10000 #평행우주 10000개
	E = 300*randn(n,N) #에러 텀
	Y = (600*X1 + 5*X2) .+ E #평행우주에서의 연봉들.. 가만 보면 행들끼리 값이 비슷함..
	#브로드 캐스팅이 되는 것도 잘 확인하기..
	B̂ = inv(X'X)X'Y
...
# ╔═╡ 77bde669-4e00-4817-b687-e319a9b4acfc
md"""실전: 의사결정 나무로..
"""
...
loss(β2,β3) = (ỹ-β2*X2-β3*X3)'*(ỹ-β2*X2-β3*X3)/n #loss가 SSE는 너무 커서,,, MSE 사용
