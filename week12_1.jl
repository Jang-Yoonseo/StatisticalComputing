...
loss(β2,β3) = (ỹ-β2*X2-β3*X3)'*(ỹ-β2*X2-β3*X3)/n #MSE..

# ╔═╡ e8d63000-f8dd-4bc9-a04a-58ed856e4711
loss(2,2),loss(2,3),loss(5,0) #마지막이 real weight.
...
begin
	β̂2s = -10:0.5:15
	β̂3s = -10:0.5:15
	p1 = plot(β̂2s,β̂3s,loss,st=:surface,colorbar=false,alpha=0.9) #3차원 그림?
	p2 = plot(β̂2s,β̂3s,loss,st=:contour,colorbar=false,levels=100) #등고선 그림.
	plot(p1,p2)
end
...
let
	@show loss(2.5,2.5)
	@show loss(5,0)
	@show loss(100,-95) #사실 이건 과적합임.. 새로운 x값에 대한 y의 예측이 잘 되지 않음.
end 
...
begin 
	l2(β̂2,β̂3) = λ*(β̂2^2 + β̂3^2) #벌점 함수..
	loss_l2(β̂2,β̂3) = loss(β̂2,β̂3) + l2(β̂2,β̂3) #최종 손실 함수..
end 
...
# ╔═╡ 51a603c7-7524-4d13-86d4-f32398dfafa8
md""" 
릿지나 라쏘 메쏘드는 추정량의 분산은 줄여주지만, 더이상 불편 추정량이 아니게 된다..
"""
...
#원래 분산이 엄청 컸음.. 막 b2,b3이 1000 까지도 넘게 나오는 경우가 있었는데..
#람다를 키워감에 따라서 분산이 줄어듦.. 하지만 더이상 불편 추정량이 아니게 됨.

# ╔═╡ cb5a3c8d-5e78-4795-a705-164ab2893d78
n
...
# ╔═╡ 9e032af5-864e-471c-af9b-e63221a962aa
md"""
그러면 어떻게 가장 괜찮은 값을 찾나요? => k-fold Cross Valdation..
"""
...
inv(X'X+0.000000000001*I)X'X*β #참값....
#inv(X'X+0.000000000001*I)X'X*[600,5,0] 도 가능..
...
let 
	function Vβ̂(λ)
		...
		#diag(추정량의 분산) 분산은 행렬로 나타나는데 diag 찍으면 벡터로 나와줌..
	end
...
end #λ 값에 따른 추정치의 분산 변화...
#갈수록 분산이 감소하는 것 보이시나요?
...
let 
	function V̂β̂(λ)
		...
		B̂ = inv(X'X+λ*I)X'Y
		j = ones(N)
		Vβ̂ = (B̂ .- B̂*j/N).^2 *j/N #브로드캐스팅의 계산 방식 잘 고민 해보기..
		return Vβ̂
	end
	λs = [1e0,1e1,1e2,1e3,1e4,1e5,1e6,1e7,1e8,1e9,1e10,1e11]
...
let 
	...
	function MSEβ̂(λ)
		...
		trVar = tr(inv(X'X+λ*I)X'X*inv(X'X+λ*I)*σ^2) #각각의 분산들을 더한 효과 
		return Bias² + trVar
	end
	...
	plot!(λs,fill(MSEols,length(λs)), label="MSE---ols", linestyle=:dash) #람다와 무관.
end #그래프 모양이 ridge 모양이라서 릿지 회귀라고 한다.....
