...
begin
	...
	df = CSV.File(download(url)) #데이터말고 변환을 분해..
...
# ╔═╡ 913a4979-8ea3-428e-8b4d-f3921aa16ba8
md""" 과정 1-3을 수행하는 것이 회전변환을 수행하는 것과 같다."""
...
1. 가죽을 반듯한 모양으로 늘리거나 줄여보며 데이터를 관찰한다. 이 때 모눈종이의 네모모양은 직사각형 혹은 정사각형이어야 한다.(사선으로 늘려서 평행사변형을 만들면 안됨.. 그건 회전 -> scaling으로 반칙임)

2. 데이터를 해석하기에 적절하다고 생각하는 어떠한 지점에서 멈춘다. 이 떄 눈금은 등간격이 아니다. 멈춘다음 모눈종이의 눈금을 모두 지운다.

3. 모눈종이의 눈금을 정사각형으로 다시 그린다.

`*` 주의: 이 해석은 모든 대각원소가 0 또는 양수일 경우에 성립함. (그런데 SVD를 하면 항상 대각선 원소가 양수임.)

"""

# ╔═╡ d7fface7-4ce8-4d12-9800-4a9f62425512
md"""
!!! warning "스케일링" 

	스케일링은 데이터가 정의되는 축의 눈금을 재설정한다. 스케일링은 역이 존재할 수도 존재하지 않을 수도 있다. 일반적으로 역이 존재하지 않는 변환을 취하는 것은 매우 위험하다. 그 이유는 잘못된 변환이 곧 데이터의 손상을 의미하기 때문이다. 따라서 역이 존재하지 않는 스케일링을 데이터에 취하는 행위는 전문가만 할 수 있다.(그런데 이렇게 위험한 변환을 왜 해야할까?) 이러한 전문가를 우리는 통계학자라고 부른다. 통계학자가 "역이 존재하지 않는 스케일링"을 사용하는 경우는 무수히 많지만 가장 많이 사용하는 경우는 아래와 같다.

    - 평균을 구하는 행위
    - y $\approx$ ŷ = Xβ̂를 만족시키는 ŷ를 구하는 행위.
...
# ╔═╡ 36f85eae-c3c4-4f03-beae-3b182ac707be
md"""
!!! warning "디커플링의 재해석" 

	두 개의 변수로 정리된 데이터가 표준정규분포를 따른다면, 중심이 밀집된 동그란 구 모양이 된다. 만약 데이터가 타원모양이라면 정규분포가 아니게 된다. 자료의 타원모양이 x축 혹은 y축이 늘어난 모양이라면 두개의 변수는 서로 분산은 다르지만 상관은 없는 분포라 해석할 수 있다. 그런데 만약에 타원이 기울기가 0이 아닌 직선 방향으로 늘어났다면 두 자료는 상관성을 가진 자료가 된다.

	하지만 타원을 적당히 돌려서 반듯한 축에 맞춘다면? 그리고 다시 축을 재설정한다면? 자료는 다시 상관없어지게 된다. 이후에 스케일링을 수행하면 분산을 맞춰주는 과정이 수행된다.
"""
...
let 
	X1,X2,X3,_,y = eachcol(iris) #시각화의 용의성을 위해..
	X = [X1 X2 X3]
	...
	V1,V2,V3 = eachcol(V)
	D̃ = Diagonal([d1,d2,0])
	Ṽ = [V1 V2 V3*0] #lotation matrix..
	iris_plot(X)
	#iris_plot(X*V) #회전
	#iris_plot(X*V*D̃) #Z축을 0으로 누름..(비가역 스케일)
	#iris_plot(X*Ṽ) #바로 윗줄이랑 똑같은 코드.
end
...
# ╔═╡ 1f4388f7-200e-4e7c-aa1a-820ad9c9c413
md""" 시험 : 직교변환에 대해 어떻게 생각하느냐... 이런 식으로 서술형을 시킬 듯..?
직교행렬이 무엇인지 아는대로 서술하시오.
